...
if(isset($_POST['submit'])){
   ...
   if(!empty($number)){
      $select_number = $conn->prepare("SELECT * FROM `users` WHERE ph_number = ?");
      $select_number->execute([$number]);
      if($select_number->rowCount() > 0){
         $message[] = 'number already taken!';
      }else{
         $update_number = $conn->prepare("UPDATE `users` SET ph_number = ? WHERE id = ?");
...
<section class="form-container update-form">
   ...
   <form action="" method="post">
      ...
      <input type="label" name="name" placeholder="<?= $fetch_profile['user_name']; ?>" class="box" maxlength="50" disabled>
      <input type="email" name="email" placeholder="<?= $fetch_profile['email']; ?>" class="box" maxlength="50" oninput="this.value = this.value.replace(/\s/g, '')">
      <input type="number" name="number" placeholder="<?= $fetch_profile['ph_number']; ?>" class="box" min="0" max="9999999999" maxlength="10">
